...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% EXAMPLE 0: Test of Gauss-Hermite rule
%
%
disp('----------------------------------------------------------------------------');
disp(' Example 0: Gauss Hermite test');
disp(' precision   nodes       value       error');

for prec=1:10

dim = 4;
domain = repmat([ 0, 1/2],[dim,1]);
tt = num2str(prec);
    ss = [blanks(6 - length(tt)),tt];
    order = 0; % not used by Gauss-Hermite rule
    
    [ weights, points ] = tsgMakeQuadrature( dim, 'gauss-hermite', 'level', prec, order, domain,0 );
    weights=weights./sum(weights);
    I = weights' * (f_test(points));
    
    E=mean(f_test(randn(1e5,dim))); %Monte-Carlo "exact" value
...
% I = weights' * f_test(points); %( exp( -points(:,1).^2 ) .* cos( points(:,2) ) ); % I is the approximated quadrature
% E = 1;%sqrt(pi);%1.861816427518323e+00; % E is the "exact" solution computed to 16 decimal places
% E = abs( I - E );
...
disp([' precision   nodes       error       nodes       error       noded       error']);
...
disp([' iteration   nodes       error       nodes       error       noded       error']);
